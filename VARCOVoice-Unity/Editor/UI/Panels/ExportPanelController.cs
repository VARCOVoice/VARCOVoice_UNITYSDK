...
namespace VARCOVoice.Editor
{
    public class ExportPanelController
    {
        ...
        private VisualElement CreateSlotUI(VarcoDialoguePlayer source, VarcoDialoguePlayer.DialogueSlot slot, int index)
        {
            ...
            row1.style.flexWrap = Wrap.NoWrap;
            ...
            row2.style.overflow = Overflow.Hidden;
            row2.style.minWidth = 0;
            row2.style.marginBottom = 6;

            var triggerLabel = new Label("Trigger");
            triggerLabel.style.width = 80;
            triggerLabel.style.flexShrink = 0;
            ...
            triggerField.style.flexShrink = 0;
            ...
            radiusField.style.flexShrink = 0;
            radiusField.style.marginRight = 16;
            ...
            visemeLabel.style.width = 50;
            visemeLabel.style.flexShrink = 0;
            ...
            row3.style.flexWrap = Wrap.NoWrap;
            row3.style.overflow = Overflow.Hidden;
            row3.style.minWidth = 0;
            row3.style.marginBottom = 6;

            var lipsyncLabel = new Label("Lipsync");
            lipsyncLabel.style.width = 80;
            lipsyncLabel.style.flexShrink = 0;
            ...
            enableLabel.style.width = 50;
            enableLabel.style.flexShrink = 0;
            enableLabel.style.marginLeft = 16;
            ...
            lipsyncToggle.style.flexShrink = 0;
            ...
            // === ACTION BAR ===
            var actions = new VisualElement();
            actions.AddToClassList("slot-action-bar"); // Moved styling to USS

            var playBtn = new Button(() => TestPlay(slot));
            playBtn.text = "▶";
            playBtn.tooltip = "Test Play";
            playBtn.AddToClassList("slot-run-btn"); // New class for Square styling
            ...
            stopBtn.AddToClassList("slot-stop-btn"); // New class
            actions.Add(stopBtn);
        ...
        private void GenerateVisemeData(VarcoDialoguePlayer source, VarcoDialoguePlayer.DialogueSlot slot)
        {
            ...
            try
            {
                ...
                slot.visemeData = analyzer.Analyze(slot.clip, 30f);
                
                EditorUtility.SetDirty(source);
                Debug.Log($"[VARCO] Generated Viseme Data: {slot.visemeData.Keyframes.Count} keyframes from '{slot.clip.name}'");
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[VARCO] Failed to generate Viseme Data: {ex.Message}");
            }
        }
        ...
        {
            var go = new GameObject("VARCOAudio_New");
            var audioSource = go.AddComponent<AudioSource>();
            audioSource.playOnAwake = false;

            var varcoSource = go.AddComponent<VarcoDialoguePlayer>();

            Undo.RegisterCreatedObjectUndo(go, "Create VARCO Audio Source");
