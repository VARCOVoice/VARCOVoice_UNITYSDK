...
namespace VARCOVoice.Editor
{
    ...
    public partial class EffectDetailController
    {
        ...
        /// </summary>
        public void SetChain(DSPChain chain)
        {
            _chain = chain;
        }
        ...
        private bool TryBuild3ZoneUI(IDSPEffect effect, HashSet<string> excluded)
        {
            ...
            {
                BuildModulationUI(chorus, "Chorus", chorus.Rate, chorus.Depth, chorus.Mix * 100f, null, null, excluded);
                return true;
            }
            if (effect is PhaserEffect phaser)
            {
                BuildModulationUI(phaser, "Phaser", phaser.Rate, phaser.Depth, phaser.Mix * 100f, phaser.Feedback, null, excluded);
                return true;
            }
            if (effect is FlangerEffect flanger)
            {
                BuildModulationUI(flanger, "Flanger", flanger.Rate, flanger.Depth, flanger.Mix * 100f, flanger.Feedback, flanger.BaseDelay, excluded);
                return true;
            }
