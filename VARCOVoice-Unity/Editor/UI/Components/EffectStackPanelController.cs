...
namespace VARCOVoice.Editor
{
    ...
    public class EffectStackPanelController
    {
        ...
        /// <summary>
        /// Update the DSP chain reference (called after Play mode exit)
        /// </summary>
        public void SetTarget(DSPChain chain)
        {
            _chain = chain;
            _selectedEffect = null;
            RebuildPillList();
            _detailController?.Clear();
            _detailController?.SetChain(chain);
            RefreshPresetDropdown();
        }
        ...
        private void QueryElements()
        {
            ...
            var randomBtn = _root.Q<Button>("random-btn");
            randomBtn?.RegisterCallback<ClickEvent>(evt =>
            {
                if (_chain == null) return;
                UnityEditor.Undo.RecordObject(_chain, "Gacha Random FX");
                _chain.ClearEffects(); // Clear first for fresh start
                DSPChainPresetLibrary.ApplySmartRandom(_chain);
                RebuildPillList();
                _detailController?.Clear();
        ...
        private void ShowAddEffectMenu()
        {
            ...
            menu.AddItem(new GUIContent("Pitch Shift"), false, () => AddEffect(new PitchShift()));
        ...
        private void ShowSavePresetDialog()
        {
            ...
            PresetNameDialog.ShowDialog("Save Preset", "My Preset", (name) => 
            {
                var preset = DSPPresetManager.SavePreset(_chain, name);
                RefreshPresetDropdown();
                
                // Show confirmation
                EditorUtility.DisplayDialog("Preset Saved", 
                    $"Preset '{name}' saved to:\nAssets/VARCOVoice/Presets", "OK");
            });
        }
