...
namespace VARCOVoice.LipSync
{
    ...
    public class EnhancedLipSyncAnalyzer
    {
        ...
        /// <summary>
        /// Smoothing factor for viseme transitions (higher = smoother but slower)
        /// </summary>
        public float Smoothing { get; set; } = 0.5f;
        
        /// <summary>
        /// Minimum time (seconds) a viseme must hold before switching
        /// </summary>
        public float MinVisemeHoldTime { get; set; } = 0.05f;
        ...
        /// </summary>
        public LipSyncData AnalyzeEnhanced(AudioClip clip, float frameRate = 24f)
        {
            ...
            for (int frame = 0; frame < totalFrames; frame++)
            {
                ...
                // Add keyframe with hysteresis (minimum hold time + significant change)
                bool shouldAddKeyframe = false;
                float timeSinceLastKeyframe = time - lastKeyframeTime;
                
                if (viseme != lastViseme && timeSinceLastKeyframe >= MinVisemeHoldTime)
                {
                    shouldAddKeyframe = true;
                }
                else if (Mathf.Abs(weight - lastWeight) > 0.2f && timeSinceLastKeyframe >= MinVisemeHoldTime)
                {
                    shouldAddKeyframe = true;
                }
        ...
        private float[] AnalyzeFrameDetailed(float[] buffer, int sampleRate)
        {
            ...
            float midFreqEnergy = CalculateMidFrequencyEnergy(_smoothedSpectrum, sampleRate);
            
            // Sibilants (ㅅ, ㅆ, ㅈ, ㅊ) - lowered threshold for better detection
            if (highFreqEnergy > 0.12f && energy > 0.05f)
            {
                float sibilantWeight = highFreqEnergy * energy * 3f;
                weights[(int)VisemeType.SS] = Mathf.Max(weights[(int)VisemeType.SS], sibilantWeight);
                weights[(int)VisemeType.CH] = sibilantWeight * 0.5f; // Also trigger CH for affricates
            }
            
            // Labial consonants (ㅁ, ㅂ, ㅍ) - detect by low frequency dominance
            if (lowFreqEnergy > midFreqEnergy * 1.5f && energy > 0.05f && highFreqEnergy < 0.15f)
            {
                float labialWeight = energy * 2f;
                weights[(int)VisemeType.PP] = Mathf.Max(weights[(int)VisemeType.PP], labialWeight);
            }
            
            // Nasal consonants (ㅁ, ㄴ, ㅇ) - detect by specific low-mid frequency pattern
            if (lowFreqEnergy > 0.1f && midFreqEnergy > 0.08f && highFreqEnergy < 0.1f && energy > 0.04f)
            {
                weights[(int)VisemeType.NN] = energy * 1.5f;
                // ㅁ also closes lips
                weights[(int)VisemeType.PP] = Mathf.Max(weights[(int)VisemeType.PP], energy * 1.2f);
            }
        ...
        private float CalculateMidFrequencyEnergy(float[] spectrum, int sampleRate)
        {
            float freqPerBin = (float)sampleRate / (spectrum.Length * 2);
            int startBin = (int)(500 / freqPerBin);
            int endBin = Mathf.Min((int)(2000 / freqPerBin), spectrum.Length - 1);
