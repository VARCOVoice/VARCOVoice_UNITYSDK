...
namespace VARCOVoice.Editor
{
    ...
    public class VarcoVoiceMainWindow : EditorWindow
    {
        ...
        {
            // When exiting Play mode, reinitialize DSP references
            if (state == PlayModeStateChange.EnteredEditMode)
            {
                // Force DSP to re-find or recreate target
                _dspPanelController?.InvalidateTarget();
                
                // Refresh Export panel list (scene objects may have changed)
                _exportPanelController?.RefreshAll();
            }
        }
        ...
        private void HandleSendToDSP(AudioClip clip)
        {
            ...
            {
                Debug.LogError("[VarcoVoiceMainWindow] DSP Panel Controller is null!");
            }
        }
