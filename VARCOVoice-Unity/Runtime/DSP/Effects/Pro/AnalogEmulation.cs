    ...
    public class TapeEmulation : DSPEffectBase
    {
        ...
        public override void Process(float[] data, int channels, int sampleRate)
        {
            ...
            for (int s = 0; s < samplesPerChannel; s++)
            {
                ...
                // Read with modulation (interpolated)
                float nominalReadPos = _delayWritePos - 64; // Fixed latency point
                float wetReadPos = nominalReadPos + totalMod;
                
                if (wetReadPos < 0) wetReadPos += _delayBufferSize;
                if (nominalReadPos < 0) nominalReadPos += _delayBufferSize;

                // 1. Nominal (Dry) Read for Phase Alignment
                int dryIdx1 = (int)nominalReadPos % _delayBufferSize;
                int dryIdx2 = (dryIdx1 + 1) % _delayBufferSize;
                float dryFrac = nominalReadPos - Mathf.Floor(nominalReadPos);
                
                float dryDelayL = _delayBuffer[0][dryIdx1] * (1f - dryFrac) + _delayBuffer[0][dryIdx2] * dryFrac;
                float dryDelayR = _delayBuffer[1][dryIdx1] * (1f - dryFrac) + _delayBuffer[1][dryIdx2] * dryFrac;

                // 2. Wet Read
                int wetIdx1 = (int)wetReadPos % _delayBufferSize;
                int wetIdx2 = (wetIdx1 + 1) % _delayBufferSize;
                float wetFrac = wetReadPos - Mathf.Floor(wetReadPos);

                left = _delayBuffer[0][wetIdx1] * (1f - wetFrac) + _delayBuffer[0][wetIdx2] * wetFrac;
                right = _delayBuffer[1][wetIdx1] * (1f - wetFrac) + _delayBuffer[1][wetIdx2] * wetFrac;
                ...
                // Output with mix
                // Use ALIGNED dry signal (dryDelayL/R) instead of immediate input (data[idxL])
                // We need to restore gain compensation for dry signal if needed, but usually dry is unity.
                // Input was multiplied by 'inputGain' before buffer. So dryDelayL is already gained.
                // We typically want Dry to be Unity relative to input? 
                // data[idxL] is original input.
                // dryDelayL is input * inputGain.
                // If we want "Mix" behavior, we usually mix Processed vs Raw.
                // But here, "Dry" must be delayed. So we use dryDelayL.
                // We should remove 'inputGain' from dryDelayL if we want true "Dry".
                // dryDelayL / inputGain? Or just accept that "Dry" here means "Through the tape mechanism but no saturation"?
                // Actually, standard Mix knob:
                // 0% Mix = Input signal (DELAYED to match).
                // 100% Mix = Processed signal.
                
                // Let's assume inputGain is part of the "Effect" applied to wet only?
                // No, line 157 applies inputGain before everything.
                // So both wet and dry in the buffer are gained.
                // To support "True Dry" (Source level), we might need to store raw input in buffer?
                // Or just divide out. division is cheap enough for 2 channels.
                // But strict rule: "Delay Compensation".
                // I will use dryDelayL / inputGain to restore original level for the Dry component.
                
                float dryL = dryDelayL;
                float dryR = dryDelayR;
                
                if (inputGain > 0.001f) // Avoid div by zero
                {
                    dryL /= inputGain;
                    dryR /= inputGain;
                }

                data[idxL] = dryL * (1f - Mix) + left * outputGain * Mix;
                if (channels > 1)
                    data[idxR] = dryR * (1f - Mix) + right * outputGain * Mix;
            }
        }
